class Utils {
    ...
    addIndexOnBundle(sdp,counter){
        var sdpLines = sdp.split('\r\n');
        var bundleIndex = -1;
        var bundle = -1;
        sdpLines.forEach((item,i) => {
            if(item.includes('a=group:BUNDLE')){
                bundleIndex = i;
                bundle = item;
            }
        });

        var bundleArr = bundle.split(" ");
        bundleArr.push(parseInt(bundleArr[bundleArr.length-1])+counter);
        sdpLines.splice(bundleIndex,1,bundleArr.join(" "));
        return sdpLines.join("\n");
    }
